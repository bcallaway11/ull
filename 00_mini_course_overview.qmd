...
[Regression based:]{.alert} @sun-abraham-2021, @wooldridge-2021
...
["Stacked" regression:]{.alert} @dube-girardi-jorda-taylor-2023
...
## Other New Approaches {#cs-advantages visibility="uncounted"}

[Important differences:]{.alert}

* CS does better with respect to including covariates (see @caetano-callaway-2024)
...
## Empirical Example: Minimum Wages and Employment {visibility="uncounted"}
...
## TWFE Regression {visibility="uncounted"}
...
## $\mathbf{\ATT(g,t)}$ (Callaway and Sant'Anna) {visibility="uncounted"}
...
## $\mathbf{\ATT^o}$ {#compute-ATTo visibility="uncounted"}
...
```{r}
#| echo: false
modelsummary(list(attO), gof_omit = ".*")
```
...
## $\mathbf{\ATT^o}$ weights {visibility="uncounted"}
...
## Additional Comments on Weights {#weights-comments visibility="uncounted"}
...
## How do we choose among identifying asssumptions? {visibility="uncounted"}
...
4. Handle covariates in different ways
    ...
    * In @caetano-callaway-2024, we consider including both changes and levels of covariates
