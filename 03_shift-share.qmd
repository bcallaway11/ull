...
[Shift-share]{.alert} (aka Bartik) designs

* [Shares]{.alert-blue} --- how exposed each unit is to each sector
* [Shifts]{.alert-green} --- aggregate, sector-level shocks
...
![](img/downes/downes-fig4.png){fig-align="center"}
...
$$s_{ijt=2} = s_{ijt=1} + \Delta s_{ij}$$

where $s_{ijt=1}$ is the share of county $i$'s employment in industry $j$ at time $t=1$ (pre-NLRA) and $\Delta s_{ij}$ is the change in that share from $t=1$ to $t=2$.

$$r_{ijt=2} = r_{jt=2} + \tilde{r}_{ijt=2}$$

where $r_{jt=2}$ is the national unionization rate in industry $j$ at time $t=2$ and $\tilde{r}_{ijt=2}$ is the unionization rate remainder in county $i$ for industry $j$ at time $t=2$.
...
$$ \E[\Delta Y \mid S_1 = s_1, S_2 = s_2] - \E[\Delta Y \mid S_1 = s_1', S_2 = s_2'] = \underbrace{\E[Y(D(s_1, s_2)) - Y(D(s_1', s_2'))]}_{\text{causal effect of shares}} $$
...
$$
\begin{aligned}
\E[\Delta Y \mid B = b+1] - \E[\Delta Y \mid B = b] &= \E[Y(D(S_1,S_2)) \mid B = b+1] - \E[Y(D(S_1,S_2)) \mid B = b] \\
\end{aligned}
$$

This holds and is easy to show from the previous assumptions.  However, the right hand side is not exactly what we want either.  The composition of shares at $B=b+1$ could be much different from the composition of shares at $B=b$, which could explain the difference in outcomes.

We can additionally assume that $D(s_1,s_2) = D(b)$ for all $s_1, s_2$ such that $s_1 r_1 + s_2 r_2 = b$.  i.e., the amount of unionization in a county is invariant to the particular shares, as long as the shift-share variable is the same, in which case we recover
...
* $Y_{it=2}(b) := Y_{it=2}(D(b))$ is the potential outcome for county $i$ at time $t=2$ if the shift-share variable were set to $b$
...
This means that we can immediately recover the causal effect of the shift-share variable on the outcome, i.e., the causal effect of differential exposure
...
* $\tau(b)$ gives more weight to units that respond more to the shift-share variable.
...
2. Are shares tailored or generic?---tailored to this channel suggests shares path, shares that could proxy many shocks suggestsshifts path

3. Which story can you say with a straight face: *"each share alone would satisfy parallel trends"* or *"this shock is as-good-as-random and only reaches units through the treatment"*?
