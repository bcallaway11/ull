...
Main reference: @callaway-goodman-santanna-2026
...
$$\mathbb{E}[Y_{t=2}(d) - Y_{t=1}(0) \mid D>0] = \mathbb{E}[Y_{t=2}(d) - Y_{t=1}(0) \mid D=d]$$
...
However, comparison of $\ATT(d\mid d)$ parameters across different doses are hard to interpret
...
* [For $\ATT^{\texttt{loc}}$:]{.alert-blue} Binarize treatment, $\ATT^o = \E[\Delta Y \mid D > 0] - \E[\Delta Y \mid D=0]$.
...
## 2. Alternative Building Block Parameters
...
## 3. Units Treated in the First Period
...
The key issue in this case is whether or not there are any [stayers]{.alert}---units that do not have change their dosage over time.
...
The previous ideas basically carry over to this case too, but there are come complications:
