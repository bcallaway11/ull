...
[Main reference:]{.alert} @callaway-goodman-santanna-2026
...
[This session:]{.alert} Move from a binary treatment to a [continuous treatment]{.alert-blue} ("dose")
...
* [Binary policy variable with differential exposure]{.alert-blue} (application in the paper: a binary Medicare policy where different hospitals had more exposure to the policy)

## What's Not a Continuous Treatment?

[Fuzzy DiD]{.alert} - binary treatment but the researcher observes aggregate data, often using policy changes as an instrument
...
* See @chaisemartin-dhaultfoeuille-2018 and @miyaji-2025

## Narrowing the Scope of the Talk

We will focus on a setting with a [panel data pre-post research design]{.alert}
...
We can view $\ATT(d \mid d)$ and $\ACRT(d \mid d)$ as the ["building blocks"]{.alert} for a more aggregated parameter.

[Aggregated versions]{.alert-blue} of these (into a single number) are
\begin{align*}
  \ATT^{\texttt{loc}} := \E\Big[\ATT(D \mid D)\Bigm|D>0\Big] \qquad \qquad \ACRT^{\texttt{loc}} := \E\Big[\ACRT(D \mid D)\Bigm|D>0\Big]
\end{align*}
...
eqlist <- list("\\ATT(d \\mid d) &= \\E[Y_{t=2}(d) - Y_{t=2}(0) \\mid D=d] \\hspace{150pt}",
  "&= \\E[Y_{t=2}(d) - Y_{t=1}(0) \\mid D=d] - \\E[Y_{t=2}(0) - Y_{t=1}(0) \\mid D=d]",
  "&= \\E[\\Delta Y \\mid D=d] - \\E[\\Delta Y \\mid D=0]")

after <- "[This is exactly what you would expect]{.alert}
...
$$\ATT^{\texttt{loc}} = \E[\Delta Y | D>0] - \E[\Delta Y | D=0]$$
...
* Plot treatment effects as a function of dose and ask: does more dose tend to increase/decrease/not affect outcomes?
...
There are [new issues]{.alert-blue} related to comparing $\ATT(d \mid d)$ at different doses and interpreting these differences as [causal effects]{.alert-green}
...
before <- "Consider comparing $\\ATT(d \\mid d)$ for two different doses

. . .

"

# Shrink only the equation block. step_by_step_eq() emits one slide per step
# and appends `after` only once, so on intermediate slides the <div> is left
# unclosed and the browser closes it at the end of the <section>. The tags go
# in Pandoc raw-HTML blocks (```{=html}) so Pandoc passes them through verbatim
# instead of parsing <div> as a native Div and trying to balance it -- an
# unclosed native Div swallows every following slide. Each raw block is built
# on a single source line because a bare ``` line would end this R chunk.
open_div  <- "```{=html}\n<div style=\"font-size: 0.8em;\">\n```\n\n"
close_div <- "```{=html}\n</div>\n```\n\n"
before <- paste0(before, open_div)

eqlist <- list("& \\ATT(d_h \\mid d_h) - \\ATT(d_l \\mid d_l) \\hspace{350pt}",
  "& \\hspace{25pt} = \\E[Y_{t=2}(d_h)-Y_{t=2}(d_l) \\mid D=d_h] + \\E[Y_{t=2}(d_l) - Y_{t=2}(0) \\mid D=d_h] - \\E[Y_{t=2}(d_l) - Y_{t=2}(0) \\mid D=d_l]",
  "& \\hspace{25pt} = \\underbrace{\\E[Y_{t=2}(d_h) - Y_{t=2}(d_l) \\mid D=d_h]}_{\\textrm{Causal Response}} + \\underbrace{\\ATT(d_l \\mid d_h) - \\ATT(d_l \\mid d_l)}_{\\textrm{Selection Bias}}")
...
* Difference-in-differences identification strategies result in $\ATT(d|d)$ parameters.  These are <span class="highlight">local parameters</span> and difficult to compare to each other
...
* Allows for selection into the treatment but no selection into the amount of treatment
...
If we maintain standard parallel trends, strong parallel trends is equivalent to a certain [restriction on treatment effect heterogeneity]{.alert}.  Notice:

"

# eqlist <- list("ATT(d \\mid d) &= \\E[Y_{t=2}(d) - Y_{t=2}(0) \\mid D=d] \\hspace{200pt} \\",
#               "&= \\E[Y_{t=2}(d) - Y_{t=1}(0) \\mid D=d] - \\E[Y_{t=2}(0) - Y_{t=1}(0) \\mid D=d] \\",
#               "&= \\E[Y_{t=2}(d) - Y_{t=1}(0) \\mid D=l] - \\E[Y_{t=2}(0) - Y_{t=1}(0) \\mid D=l] \\",
#               "&= \\E[Y_{t=2}(d) - Y_{t=2}(0) \\mid D=l] = ATT(d|l)")

eqlist <- list("\\ATT(d|d) &= \\E[Y_{t=2}(d) - Y_{t=1}(0) \\mid D=d] - \\E[Y_{t=2}(0) - Y_{t=1}(0) \\mid D=d] \\hspace{200pt} \\",
  "&= \\E[Y_{t=2}(d) - Y_{t=1}(0) \\mid D>0] - \\E[Y_{t=2}(0) - Y_{t=1}(0) \\mid D > 0] \\",
  "&= \\E[Y_{t=2}(d) - Y_{t=2}(0) \\mid D>0] = \\ATT(d)")
...
$$
\\begin{aligned}
  \\ACRT(d) := \\frac{\\partial \\ATT(d)}{\\partial d} \\qquad &\\textrm{or} \\qquad \\ACRT^{\\texttt{glob}} := \\E\\Big[\\ACRT(D) \\Big| D>0\\Big]
\\end{aligned}
$$
...
\end{align*}
where $m_\Delta(l) := \E[\Delta Y \mid D=l] - \E[\Delta Y \mid D=0]$ and $w(l)$ are weights
...
* [For $\ATT^{\texttt{loc}}$:]{.alert-blue} Binarize treatment, $\ATT^{\texttt{loc}} = \E[\Delta Y \mid D > 0] - \E[\Delta Y \mid D=0]$.
* [For $\ATT(d \mid d)$:]{.alert-blue} Nonparametrically estimate $m_\Delta(d) = \E[\Delta Y \mid D=d]-\E[\Delta Y \mid D=0]$
...
* [For $\ACRT(d)$:]{.alert-blue} Under strong parallel trends, derivative is equal to $\ACRT(d)$
* [For $\ACRT^{\texttt{glob}}$:]{.alert-blue} Average $\ACRT(D)$ over $D>0$; i.e., $\ACRT^{\texttt{glob}} = \E[\ACRT(D)|D>0]$
...
[Application:]{.alert} Simplified version of @acemoglu-finkelstein-2008
...
[Focus:]{.alert} Comparison of estimates of $\ACRT^{\texttt{glob}}$ to $\beta^{twfe}$

* They are both weighted averages of $\ACRT(d)$, only the weights differ

## Empirical Application

This is a simplified version of @acemoglu-finkelstein-2008
...
# 5. Extensions {visibility="uncounted"}

## 1. Multiple Periods and Variation in Treatment Timing

Staggered treatment adoption has received a lot of attention in the literature on DiD with a binary treatment [@goodman-bacon-2021; @chaisemartin-dhaultfoeuille-2020; @callaway-santanna-2021]
...
* Following @callaway-santanna-2021, can focus on identifying "group-time-dose" parameters, e.g., $\ATT(d, g, t \mid d, g)$ (under parallel trends) or $\ACRT(d, g, t \mid g)$ (under strong parallel trends)
...
Including covariates in the parallel trends assumption often makes it more plausible [@heckman-ichimura-todd-1997;@abadie-2005;@caetano-callaway-2025]
...
@haddad-huber-zhang-2024 use kernel-based estimators combined with double/debiased machine learning, allowing flexible, data-adaptive adjustment for covariates.
...
This is the average treatment effect per unit of dose.  This parameter shows up in [@chaisemartin-ciccia-dhaultfoeuille-knau-2024;@chaisemartin-dhaultfoeuille-pasquier-sow-vazquez-2025].
...
The key issue in this case is whether or not there are any [stayers]{.alert}---units that do not change their dose over time.
...
* If yes, then the previous results effectively carry over to this case.  You can proceed by, for each initial dose, applying the same sort of arguments as above but with initial dose as the "untreated" group.
  ...
  * See @chaisemartin-dhaultfoeuille-pasquier-sow-vazquez-2025 for one approach.
...
* [Link to paper:]{.alert} [[published](https://www.aeaweb.org/articles?id=10.1257/aer.20240137)] [[arXiv](https://arxiv.org/abs/2107.02637)]

* [Code]{.alert-blue}: R `contdid` package available on CRAN
